...
static struct lsm6dx0_pinctrl_info lsm6dx0_pctrl;

#define LSM6DX0_PINCTRL_STATE_SLEEP    "lsm6ds0_sleep"
#define LSM6DX0_PINCTRL_STATE_DEFAULT  "lsm6ds0_default"

static int lsm6dx0_acc_gyr_pinctrl_init(struct device *dev)
{
	SENSOR_LOG_ERROR("enter\n");

	lsm6dx0_pctrl.pinctrl = devm_pinctrl_get(dev);

	if (IS_ERR_OR_NULL(lsm6dx0_pctrl.pinctrl)) {
		pr_err("%s:%d Getting pinctrl handle failed\n",
			__func__, __LINE__);
		return -EINVAL;
	}
	lsm6dx0_pctrl.gpio_state_active = pinctrl_lookup_state(
					       lsm6dx0_pctrl.pinctrl,
					       LSM6DX0_PINCTRL_STATE_DEFAULT);

	if (IS_ERR_OR_NULL(lsm6dx0_pctrl.gpio_state_active)) {
		pr_err("%s:%d Failed to get the active state pinctrl handle\n",
			__func__, __LINE__);
		return -EINVAL;
	}
	lsm6dx0_pctrl.gpio_state_suspend = pinctrl_lookup_state(
						lsm6dx0_pctrl.pinctrl,
						LSM6DX0_PINCTRL_STATE_SLEEP);

	if (IS_ERR_OR_NULL(lsm6dx0_pctrl.gpio_state_suspend)) {
...
static struct device_attribute lsm6dx0_acc_attributes[] = {
	...
	__ATTR(pollrate_ms, 0644, attr_polling_rate_acc_show, attr_polling_rate_acc_store),
	__ATTR(enable_device, 0644, attr_enable_acc_show, attr_enable_acc_store),
	__ATTR(range, 0644, attr_range_acc_show,	attr_range_acc_store),
	__ATTR(anti_aliasing_frequency,  0644, attr_aa_filter_show, attr_aa_filter_store),
...
static struct device_attribute lsm6dx0_gyr_attributes[] = {
	...
	__ATTR(pollrate_ms, 0644, attr_polling_rate_gyr_show, attr_polling_rate_gyr_store),
	__ATTR(enable_device, 0644, attr_enable_gyr_show, attr_enable_gyr_store),
		...
		const struct i2c_device_id *id)
{
	...
	lsm6dx0_acc_gyr_pinctrl_init(&client->dev);
	err = pinctrl_select_state(lsm6dx0_pctrl.pinctrl,
			lsm6dx0_pctrl.gpio_state_active);
	if (err)
	{
	...
	if (err < 0) {
		dev_err(&client->dev, "failed to create_class_sysfs_interfaces\n");
...
static int lsm6dx0_acc_gyr_resume(struct i2c_client *client)
{
	struct lsm6dx0_status *stat = i2c_get_clientdata(client);
	int ret = 0;
	...
	{
		SENSOR_LOG_ERROR("schedule acc work\n");
		hrtimer_start(&stat->hr_timer_acc, stat->ktime_acc, HRTIMER_MODE_REL);
	}

	if(atomic_read(&stat->enabled_gyr) == 1)
	{
		SENSOR_LOG_ERROR("schedule gyro work\n");
		hrtimer_start(&(stat->hr_timer_gyr), stat->ktime_gyr, HRTIMER_MODE_REL);
    }
...
static int lsm6dx0_acc_gyr_suspend(struct i2c_client *client, pm_message_t mesg)
{
	struct lsm6dx0_status *stat = i2c_get_clientdata(client);
	int ret = 0;
	...
	if(atomic_read(&stat->enabled_acc) == 1)
	{
		SENSOR_LOG_ERROR("cancel acc work\n");
		cancel_work_sync(&stat->input_work_acc);
	...
	if(atomic_read(&stat->enabled_gyr) == 1)
	{
		SENSOR_LOG_ERROR("cancel gyro work\n");
		cancel_work_sync(&stat->input_work_gyr);
